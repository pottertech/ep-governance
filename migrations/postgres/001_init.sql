-- EP-Governance PostgreSQL initial schema migration
-- Version: v1.1.1
-- All XID columns use TEXT. Timestamps use TIMESTAMPTZ. Structured data uses JSONB.
--
-- Table creation order respects FK dependencies:
--   ep_projects → ep_lattices → ep_principals → ep_branches → ep_nodes → ep_edges
--   → ep_policies → ep_roles → ep_role_bindings → ep_credentials → ep_transitions
--   → ep_authorizations → ep_approvals → ep_risk → ep_audit → ep_work → ep_sessions
--   → ep_transfer → ep_policy_versions
--
-- The only circular FK (ep_branches.head_node_id → ep_nodes.id) is added via
-- ALTER TABLE after ep_nodes is created.

-- ============================================================================
-- Identity (created early — many tables reference ep_principals)
-- ============================================================================
...
-- ep_branches created without head_node_id FK (circular dep with ep_nodes)
CREATE TABLE ep_branches (
    id           TEXT PRIMARY KEY,
    lattice_id   TEXT NOT NULL REFERENCES ep_lattices(id),
    name         TEXT NOT NULL,
    head_node_id TEXT,  -- FK added via ALTER TABLE after ep_nodes is created
...
CREATE TABLE ep_nodes (
    ...
    description        TEXT,
    bt_planning_budget FLOAT CHECK (bt_planning_budget >= 0),
...
-- Now add the circular FK: ep_branches.head_node_id → ep_nodes(id)
ALTER TABLE ep_branches
    ADD CONSTRAINT ep_branches_head_node_id_fkey
        FOREIGN KEY (head_node_id) REFERENCES ep_nodes(id);
...
CREATE TABLE ep_policies (
    ...
    exception_to       JSONB NOT NULL DEFAULT '[]'::jsonb,
...
CREATE TABLE ep_transitions (
    ...
    pulse_trace                   JSONB NOT NULL DEFAULT '[]'::jsonb,
...
-- Per-lattice audit head tracking. Only ep_service role can modify (see 002_roles.sql).
-- No UPDATE/DELETE prevention triggers — immutability enforced via database roles.
...
-- Immutable append-only event log. Only ep_service role can INSERT.
-- No UPDATE or DELETE permitted (enforced via database roles, not triggers).
...
CREATE UNIQUE INDEX idx_ep_work_claims_active ON ep_work_claims (branch_id, region) WHERE status = 'active';
...
CREATE TABLE ep_transfer_packages (
    ...
    lattice_state     JSONB NOT NULL DEFAULT '{}'::jsonb,
    model_info        TEXT,
    created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
